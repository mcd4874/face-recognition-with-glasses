...
# Train = np.zeros([size_label, size_label, 1, 1])
data = []
label = []
...
for root, dirs, files in os.walk(dirpath):
    for file in files:
        ...
        labelFile= file.split('label_')[1]
        img = cv.imread(dirpath + file)
        # print(img.shape)
        label_img = cv.imread(dirLabel+labelFile)
        # plt.imshow(img)
        # plt.title("origin")
        # plt.show()
        ...
        label_img = cv.cvtColor(label_img, cv.COLOR_BGR2YCR_CB)
        label_img = im2double(label_img)  # Only use the luminance value.

        # Create groundtruth and baseline image.
        im_input = modcrop(img)
        size = im_input.shape
        h = size[0]
        w = size[1]
        # print(size)

        im_label = modcrop(label_img)
        ...
        for x in range(0, h - size_input, stride):
            for y in range(0, w - size_input, stride):
                ...
                subim_label = im_label[int(x + padding): int(x + padding + size_label),
                              int(y + padding): int(y + padding + size_label)]

                # plt.imshow(subim_input)
                # plt.show()
                # plt.imshow(subim_label)
                # plt.show()
...
print('Train shape is', label.shape)
