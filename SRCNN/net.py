...
def SRCNN(x):
	...
	weights = {'w1' : tf.Variable(tf.random_normal([9, 9, 1, 64], stddev=1e-3)),
			   'w2' : tf.Variable(tf.random_normal([9, 9, 64, 32], stddev=1e-3)),
			   'w3' : tf.Variable(tf.random_normal([1, 1, 32, 1], stddev=1e-3))}
	...
	conv1 = parametric_relu(conv2d(x, weights['w1']) + biases['b1'])
	# conv2 = tf.nn.relu(conv2d(conv1, weights['w2']) + biases['b2'])
	conv2 = parametric_relu(conv2d(conv1, weights['w2']) + biases['b2'])
...
# 	model = SRCNN(x)
# 	l2_loss = tf.reduce_mean(tf.square(labels - model))
# 	optimizer = tf.TrainLabel.GradientDescentOptimizer(learning_rate).minimize(l2_loss)
...
# def generate_h5():
# 	input_setup(True,size_input,size_label)
# generate_h5()
train_SRCNN(images)
# To gener aten, need to create data in h5 format then feed.